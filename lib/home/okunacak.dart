...
class Oku extends StatefulWidget {
  const Oku({super.key});

  @override
  State<Oku> createState() => _OkuState();
}

class _OkuState extends State<Oku> {
  TextEditingController okunacakController = TextEditingController();
            ...
            Container(
              decoration: const BoxDecoration(
                image: DecorationImage(
                  image: AssetImage('lib/assets/indir (6).jpg'),
                  fit: BoxFit.cover,
                ),
              ),
  ...
  FloatingActionButton floatingActionButtonWidget(BuildContext context) {
    return FloatingActionButton(
      onPressed: () {
        okunacakController.clear();
        showDialog(
          context: context,
          builder: (context) => AlertDialog(
            content: TextFormField(
              controller: okunacakController,
            ...
            actions: [
              TextButton(
                  onPressed: () {
                    if (okunacakController.text.isEmpty) {
                      showDialog(
                        context: context,
                        builder: (context) => AlertDialog(
                          title: Text("UYARI"),
